...
reload_module(custom_evaluate)                       
reload_module(custom_metrics)                                  
reload_module(custom_callbacks)
reload_module(custom_optimizer)
...
#------------------------------------------------------------------------------
take_only         = 1                               # float, [0 1], % of the total data you want to keep
input_shape       = (72,8,8,4)                        # tuple, the shape of the input data
time_dir          = sys.argv[1]                       #str(time.time())    # string, Name of result directory set to "str(time.time())", for multiple runs 
...
num_epochs         = 75                             # int,   >= 1 , Number of training data epochs
batch_perc         = .05                             # float, [0 1], Percentage of training to use data per training batch (1 = 100%)
...
for fold_num in range(0,np.size(u_inst)):
    ...
    bad_out_ind  =  np.setdiff1d(test_ind,bad_out_ind)
    good_out_ind = np.setdiff1d(test_ind,good_out_ind)

    #Take a random 50% of the remaining subjects and split them into training and validation sets.
    train_bad_out_ind    = np.random.choice(bad_out_ind, int(np.size(bad_out_ind)*(train_perc/(val_perc+train_perc))),replace=False)
    train_good_out_ind   = np.random.choice(good_out_ind, int(np.size(good_out_ind)*(train_perc/(val_perc+train_perc))),replace=False)
    ...
    #For each n epochs...
    for i in range(checkpoint_period,epochs_completed,checkpoint_period):
